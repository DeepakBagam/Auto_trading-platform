...
def test_v3_interval_params_support_full_upstox_units(monkeypatch) -> None:
    monkeypatch.setattr(
        "backend.data_layer.collectors.upstox_collector.get_settings",
        lambda: SimpleNamespace(
            upstox_base_url="https://api.upstox.com",
    ...
    assert collector._history_interval_params("1m") == ("minutes", "1")
    assert collector._history_interval_params("300minute") == ("minutes", "300")
    assert collector._history_interval_params("5h") == ("hours", "5")
    assert collector._history_interval_params("day") == ("days", "1")
    assert collector._history_interval_params("week") == ("weeks", "1")
    assert collector._history_interval_params("month") == ("months", "1")


def test_full_backfill_uses_upstox_availability_start_dates(monkeypatch) -> None:
    monkeypatch.setattr(
        "backend.data_layer.collectors.upstox_collector.get_settings",
        lambda: SimpleNamespace(
            upstox_base_url="https://api.upstox.com",
    ...
    collector = UpstoxCollector()
    seen: dict[str, tuple[date, int]] = {}

    def fake_ingest(db, instrument_key, interval, from_date, to_date, chunk_days):
        del db, instrument_key, to_date
        seen[interval] = (from_date, chunk_days)
        return {"inserted": 0, "records_seen": 0, "chunks": 0, "errors": 0}

    monkeypatch.setattr(collector, "_ingest_interval_range_chunked", fake_ingest)
    monkeypatch.setattr(collector, "_mark_freshness", lambda *args, **kwargs: None)

    class DummyDb:
        def commit(self):
            return None

    collector.ingest_historical_full(DummyDb())

    assert seen["1minute"] == (date(2022, 1, 1), 31)
    assert seen["15minute"] == (date(2022, 1, 1), 31)
    assert seen["16minute"] == (date(2022, 1, 1), 92)
    assert seen["1hour"] == (date(2022, 1, 1), 92)
    assert seen["day"] == (date(2000, 1, 1), 3652)
    assert seen["week"] == (date(2000, 1, 1), 100_000)
    assert seen["month"] == (date(2000, 1, 1), 100_000)
