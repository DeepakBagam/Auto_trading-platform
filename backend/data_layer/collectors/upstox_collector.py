...
from backend.data_layer.processors.candle_resampler import resample_candles
...
from backend.utils.constants import IST_ZONE, SUPPORTED_INTERVALS
from backend.utils.intervals import interval_backfill_start, interval_chunk_days, interval_to_upstox_v3_params, normalize_interval
...
class UpstoxCollector:
    ...
    V2_INTERVALS = {"1minute", "30minute", "day", "week", "month"}
    ...
    def fetch_historical_candles(
        ...
        if version_pref in {"v2", "auto"} and interval in self.V2_INTERVALS:
    ...
    def _history_interval_params(self, interval: str) -> tuple[str, str]:
        return interval_to_upstox_v3_params(interval)
    ...
    def persist(self, db: Session, records: List[CandleRecord], update_existing: bool = False) -> int:
        ...
        for raw_record in records:
            ...
            key = (normalized.instrument_key, normalized.interval, normalized.ts)
            existing = deduped.get(key)
            if existing is None or float(normalized.volume or 0.0) >= float(existing.volume or 0.0):
    ...
    def ingest_historical_batch(self, db: Session, days_back: int = 180) -> dict:
        ...
        for instrument_key in self.settings.data_ingestion_instrument_keys:
    ...
    def ingest_historical_full(
        self,
        db: Session,
        one_minute_days: int = 730,
        intervals: Iterable[str] | None = None,
        instrument_keys: Iterable[str] | None = None,
    ) -> dict:
        now = datetime.now(IST_ZONE).date()
        del one_minute_days
        target_intervals = tuple(normalize_interval(item) for item in intervals) if intervals else SUPPORTED_INTERVALS
        target_instruments = tuple(instrument_keys) if instrument_keys else tuple(self.settings.data_ingestion_instrument_keys)
        summary: dict[str, dict] = {}
        for instrument_key in target_instruments:
            for interval in target_intervals:
                from_date = interval_backfill_start(interval)
